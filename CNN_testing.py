...
def main():
    ...
    fname, X_test, Y_test, nb_classes = build_dataset(
        "{}/testing".format(data_dir), args.dimension)
    print("num of classes : {}".format(nb_classes))

    model = load_model('{}epochs_{}batch_cnn_model_{}.h5'.format(
            epochs, batch_size, data_dir.replace("/", "_")))

    predicted = model.predict(X_test)

    f_predict = open('predict_result.txt','a')
    f_predict.write('==========\n')
    f_predict.write('{}epochs_{}batch_cnn\n'.format(epochs, batch_size))
    for i in range(len(predicted)) :
        f_predict.write('code : {} - {} - {}\n'.format(fname[i],predicted[i],Y_test[i]))
    f_predict.write('==========\n')

    y_pred = []
    for i in range(len(predicted)) :
        if predicted[i][1] > 0.60:
            y_pred.append(1)
        else:
            y_pred.append(0)
    ...
    f_predict = open('predict_result_argmax.txt', 'a')
    f_predict.write('==========\n')
    f_predict.write('{}epochs_{}batch_cnn\n'.format(epochs, batch_size))
    for i in range(len(y_pred)) :
        if y_pred[i] == 1 :
            f_predict.write('code : {} - {} - {}\n'.format(fname[i],y_pred[i],Y_test[i]))
    f_predict.write('==========\n')
